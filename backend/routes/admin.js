...
// GET /api/admin/sandbox-balance - read M-Pesa sandbox ledger
router.get('/sandbox-balance', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT balance, updated_at FROM mpesa_sandbox_ledger WHERE id = 1'
    );
    ...
    console.error(err);
    res.status(500).json({ error: 'Could not fetch sandbox balance' });
  }
});

// PATCH /api/admin/sandbox-balance/reset - reset sandbox balance to 500 billion
router.patch('/sandbox-balance/reset', async (req, res) => {
  const newBalance = req.body.balance ?? 500000000000;
  try {
    await pool.query(
      'UPDATE mpesa_sandbox_ledger SET balance = $1, updated_at = now() WHERE id = 1',
      [newBalance]
    );
    res.json({ message: 'Sandbox balance reset', balance: newBalance });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Could not reset sandbox balance' });
  }
});
